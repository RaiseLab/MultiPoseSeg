...
def get_losses(ground_truth, outputs):
    ...
    losses.append(
        long_offset_loss(long_offset_true, long_offsets, seg_true, crowd_mask, unannotated_mask, overlap_mask))
...
def train(load_pretrained_model=True, checkpoint_path=None):
    ...
    ground_truth = [kp_maps_true, short_offsets_true, mid_offsets_true, long_offsets_true, seg_mask_true, crowd_mask,
                    unannotated_mask, overlap_mask]
    loss = get_losses(ground_truth, outputs)
    print("[*]\tModel Build Finished!")
    print(count1())
    # the ResNet parameters############# Need to be change if want to run Resnet 101#################
    ...
    for n in range(config.NUM_EPOCHS):
        for m in range(config.NUM_EPOCHS_SIZE):
            ...
            feed_dict = {img: batch[0], kp_maps_true: batch[1], short_offsets_true: batch[2],
                         mid_offsets_true: batch[3], long_offsets_true: batch[4],
                         seg_mask_true: batch[5], crowd_mask: batch[6], unannotated_mask: batch[7],
                         overlap_mask: batch[8]}
...
checkpoint_path = '/content/drive/My Drive/StrongPose/model/StrongPose/model.ckpt-11'
train()
