...
checkpoint_path = 'model/StrongPose/model.ckpt-193'  ##'./content/drive/My Drive/StrongPose/model/StrongPose/'+'model.ckpt-11'
...
for i in range(len(multiscale)):
    scale = multiscale[i]
    scale_img = dataset.get_multi_scale_img(give_id=1818, scale=scale)  # 566282
    if i == 0:
        img = scale_img[:, :, [2, 1, 0]]
...
offsets = sample_output[0][:, :, 10]
plt.imsave(save_path + 'B_SKHM.jpg', offsets)

Tune = gaussian_filter(H, sigma=0.1)

for i in range(17):
    H[:, :, i] = gaussian_filter(H[:, :, i], sigma=0.1)
...
# visualize_short_offsets(offsets=sample_output[1], heatmaps=H, keypoint_id='Rknee', img=img, every=2,save_path=save_path)
...
# pred_kp = gaussian_filter(pred_kp, sigma=0.5)
...
cv2.putText(result, ('Detected humans: {}'.format(len(pred_skels))), (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
           (255, 255, 255), 1)
...
#visualize_short_offsets(offsets=sample_output[1], heatmaps=H, keypoint_id='Rshoulder', img=result, every=2,
#                        save_path=save_path)

offsets = sample_output[1][:, :, 10]
plt.imsave(save_path + 'keypoint_binary_offset.jpg', offsets)
...
plt.imsave(save_path + 'segmentation_mask.jpg', apply_mask(img, sample_output[4][:, :, 0] > 0.1,
                                                           color=[0, 0, 0]))  ###use for producing dark color mask######

seg_prob = sample_output[4][:, :, 0] > 0.4
plt.imsave(save_path + 'Binary_Mask.jpg', seg_prob)

# plt.imsave(save_path+'Segmentation_Mask.png',apply_mask(img, sample_output[4][:,:,0]>0.5, color=[255,255,235]))


visualize_long_offsets(offsets=sample_output[3], keypoint_id='Rhip', seg_mask=sample_output[4], img=img_img,
                       every=40, save_path=save_path)

offsets = sample_output[3][:, :, 10]
plt.imsave(save_path + 'mask_binary_offset.jpg', offsets)
