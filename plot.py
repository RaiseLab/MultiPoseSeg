...
def visualize_short_offsets(offsets, keypoint_id, centers=None, heatmaps=None, radius=config.KP_RADIUS, img=None,
    ...
    plt.quiver(J, I, kp_offsets[I, J, 0], kp_offsets[I, J, 1], color='b', angles='xy', scale_units='xy', scale=2)
    plt.savefig('./demo_result/Keypoint_Offsets.png', bbox_inches='tight')
...
def visualize_mid_offsets(offsets, from_kp, to_kp, centers=None, heatmaps=None, radius=config.KP_RADIUS, img=None,
    ...
    plt.rcParams['savefig.dpi'] = 300
    plt.rcParams['figure.dpi'] = 200
    plt.quiver(J, I, kp_offsets[I, J, 0], kp_offsets[I, J, 1], color='r', angles='xy', scale_units='xy', scale=2)
    plt.savefig(save_path + 'middle_offsets.png', bbox_inches='tight')
...
def visualize_long_offsets(offsets, keypoint_id, seg_mask, img=None, every=1, save_path='./'):
    ...
    I, J = np.nonzero(mask)
    ######### To present with Mask or not comment it ###############
    if img is not None:
        plt.imshow(img)
    ##############################################
    # plt.rcParams['savefig.dpi'] = 300
    # plt.rcParams['figure.dpi'] = 200
    plt.quiver(J, I, kp_offsets[I, J, 0], kp_offsets[I, J, 1], color='b', angles='xy', scale_units='xy', scale=1)
    plt.savefig(save_path + 'Mask_Offset.png', bbox_inches='tight')
...
# def Seg_offset(offsets, keypoint_id, img=None, every=1,save_path='./'):
#    if isinstance(keypoint_id, str):
#        if not keypoint_id in config.KEYPOINTS:
#            raise ValueError('{} not a valid keypoint name'.format(keypoint_id))
#        else:
...
#    plt.rcParams['savefig.dpi'] = 300
#    plt.rcParams['figure.dpi'] = 200
#    plt.quiver(kp_offsets[0], kp_offsets[1], color='r', angles='xy', scale_units='xy', scale=1)        #J, I,
#    plt.savefig(save_path+'seg_offsets.png',bbox_inches = 'tight')
...
def plot_poses(img, skeletons, save_path=None):
    ...
    for i in range(17):
        ...
        for j in range(len(skeletons)):
            cv.circle(canvas, tuple(skeletons[j][i, 0:2].astype('int32')), 2, colors[i], thickness=5)
    ...
    for i in range(config.NUM_EDGES):
        for j in range(len(skeletons)):
            ...
            # stop=time.time()
            # print(stop-start)
            plt.imsave(save_path + 'Pose.png', canvas[:, :, :])
            fig = matplotlib.pyplot.gcf()


################################################Instance Segmentation#########################################################

def apply_mask(image, mask, color, alpha=0.2):  ### aplha=1 to make dark mask/ aplpha=0.3 to make color mask########
    #    image = image.copy()

    for c in range(3):
        image[:, :, c] = np.where(mask == 1,
                                  gaussian_filter(image[:, :, c], sigma=0.1) *
                                  (1 - alpha) + alpha * color[c] * 255,  ####   Tune = gaussian_filter(H, sigma=1)
                                  image[:, :, c])
    return image
...
def plot_instance_masks(masks, img, save_path='./'):
    ...
    for mask in masks:
        ...
        canvas = apply_mask(img, mask, color, alpha=0.6)
    plt.imsave(save_path + 'Instances_Masks.png', canvas)
    ...
    for i in range(config.NUM_EDGES):
        for j in range(len(skeletons)):
            ...
            for mask in masks:
                ...
                plt.imsave(save_path + 'Seg_Pose.png', canvass[:, :, :])
                fig = matplotlib.pyplot.gcf()
...
def BHM_Pose(img, skeletons, save_path=None):
    colors = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0], \
              [0, 255, 85], [0, 255, 170], [0, 255, 255], [0, 170, 255], [0, 85, 255], [0, 0, 255], [85, 0, 255], \
              [170, 0, 255], [255, 0, 255], [255, 0, 170], [255, 0, 85]]
    cmap = matplotlib.cm.get_cmap('hsv')
    ...
    plt.imshow(to_plot[:, :, [2, 1, 0]])
    fig = matplotlib.pyplot.gcf()
    plt.imsave(save_path + 'BHM_Keypoints.png', to_plot[:, :, [2, 1, 0]])
    ...
    for i in range(config.NUM_EDGES):
        for j in range(len(skeletons)):
            ...
            plt.imsave(save_path + 'BHM_Pose.png', canvas[:, :, :])
            fig = matplotlib.pyplot.gcf()
